  ...
  const [searchTimestamps, setSearchTimestamps] = useState<number[]>(() => {
    ...
    if (storedTimestamps) {
      try {
        ...
        console.error(
          "Error parsing search timestamps from localStorage:",
          error,
        );
        return [];
      }
        ...
        <nav className="sticky top-0 z-50 backdrop-blur-md bg-white/70 dark:bg-gray-900/70 border-b border-gray-200/50 dark:border-gray-800/50">
          <div className="max-w-6xl mx-auto px-4 py-4">
            <div className="flex items-center justify-between">
              ...
              {isDesktop ? (
                <div className="flex items-center gap-3">{navLinks}</div>
              ) : (
                <Sheet>
                  ...
                  <SheetContent>
                    <div className="flex flex-col gap-4 py-6">{navLinks}</div>
                  </SheetContent>
                </Sheet>
              )}
            </div>
